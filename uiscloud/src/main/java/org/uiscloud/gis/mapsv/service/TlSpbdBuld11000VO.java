package org.uiscloud.gis.mapsv.service;


public class TlSpbdBuld11000VO {
	
	private Integer gid;
	
	private String bdtypCd;
	
	private String bdMgtSn;
	
	private Integer bsiIntSn;
	
	private String bsiZonNo;
	
	private Integer buldMnnm;
	
	private String buldNm;
	
	private String buldNmDc;
	
	private String buldSeCd;
	
	private Integer buldSlno;
	
	private String bulDpnSe;
	
	private String bulEngNm;
	
	private Integer bulManNo;
	
	private String emdCd;
	
	private Integer eqbManSn;
	
	private Integer groFloCo;
	
	private String liCd;
	
	private Integer lnbrMnnm;
	
	private Integer lnbrSlno;
	
	private String mntnYn;
	
	private String mvmnDe;
	
	private String mvmnResn;
	
	private String mvmResCd;
	
	private String ntfcDe;
	
	private String opertDe;
	
	private String posBulNm;
	
	private Double rdsManNo;
	
	private String rnCd;
	
	private String sigCd;
	
	private Integer undFloCo;

	public Integer getGid() {
		return gid;
	}

	public void setGid(Integer gid) {
		this.gid = gid;
	}

	public String getBdtypCd() {
		return bdtypCd;
	}

	public void setBdtypCd(String bdtypCd) {
		this.bdtypCd = bdtypCd;
	}

	public String getBdMgtSn() {
		return bdMgtSn;
	}

	public void setBdMgtSn(String bdMgtSn) {
		this.bdMgtSn = bdMgtSn;
	}

	public Integer getBsiIntSn() {
		return bsiIntSn;
	}

	public void setBsiIntSn(Integer bsiIntSn) {
		this.bsiIntSn = bsiIntSn;
	}

	public String getBsiZonNo() {
		return bsiZonNo;
	}

	public void setBsiZonNo(String bsiZonNo) {
		this.bsiZonNo = bsiZonNo;
	}

	public Integer getBuldMnnm() {
		return buldMnnm;
	}

	public void setBuldMnnm(Integer buldMnnm) {
		this.buldMnnm = buldMnnm;
	}

	public String getBuldNm() {
		return buldNm;
	}

	public void setBuldNm(String buldNm) {
		this.buldNm = buldNm;
	}

	public String getBuldNmDc() {
		return buldNmDc;
	}

	public void setBuldNmDc(String buldNmDc) {
		this.buldNmDc = buldNmDc;
	}

	public String getBuldSeCd() {
		return buldSeCd;
	}

	public void setBuldSeCd(String buldSeCd) {
		this.buldSeCd = buldSeCd;
	}

	public Integer getBuldSlno() {
		return buldSlno;
	}

	public void setBuldSlno(Integer buldSlno) {
		this.buldSlno = buldSlno;
	}

	public String getBulDpnSe() {
		return bulDpnSe;
	}

	public void setBulDpnSe(String bulDpnSe) {
		this.bulDpnSe = bulDpnSe;
	}

	public String getBulEngNm() {
		return bulEngNm;
	}

	public void setBulEngNm(String bulEngNm) {
		this.bulEngNm = bulEngNm;
	}

	public Integer getBulManNo() {
		return bulManNo;
	}

	public void setBulManNo(Integer bulManNo) {
		this.bulManNo = bulManNo;
	}

	public String getEmdCd() {
		return emdCd;
	}

	public void setEmdCd(String emdCd) {
		this.emdCd = emdCd;
	}

	public Integer getEqbManSn() {
		return eqbManSn;
	}

	public void setEqbManSn(Integer eqbManSn) {
		this.eqbManSn = eqbManSn;
	}

	public Integer getGroFloCo() {
		return groFloCo;
	}

	public void setGroFloCo(Integer groFloCo) {
		this.groFloCo = groFloCo;
	}

	public String getLiCd() {
		return liCd;
	}

	public void setLiCd(String liCd) {
		this.liCd = liCd;
	}

	public Integer getLnbrMnnm() {
		return lnbrMnnm;
	}

	public void setLnbrMnnm(Integer lnbrMnnm) {
		this.lnbrMnnm = lnbrMnnm;
	}

	public Integer getLnbrSlno() {
		return lnbrSlno;
	}

	public void setLnbrSlno(Integer lnbrSlno) {
		this.lnbrSlno = lnbrSlno;
	}

	public String getMntnYn() {
		return mntnYn;
	}

	public void setMntnYn(String mntnYn) {
		this.mntnYn = mntnYn;
	}

	public String getMvmnDe() {
		return mvmnDe;
	}

	public void setMvmnDe(String mvmnDe) {
		this.mvmnDe = mvmnDe;
	}

	public String getMvmnResn() {
		return mvmnResn;
	}

	public void setMvmnResn(String mvmnResn) {
		this.mvmnResn = mvmnResn;
	}

	public String getMvmResCd() {
		return mvmResCd;
	}

	public void setMvmResCd(String mvmResCd) {
		this.mvmResCd = mvmResCd;
	}

	public String getNtfcDe() {
		return ntfcDe;
	}

	public void setNtfcDe(String ntfcDe) {
		this.ntfcDe = ntfcDe;
	}

	public String getOpertDe() {
		return opertDe;
	}

	public void setOpertDe(String opertDe) {
		this.opertDe = opertDe;
	}

	public String getPosBulNm() {
		return posBulNm;
	}

	public void setPosBulNm(String posBulNm) {
		this.posBulNm = posBulNm;
	}

	public Double getRdsManNo() {
		return rdsManNo;
	}

	public void setRdsManNo(Double rdsManNo) {
		this.rdsManNo = rdsManNo;
	}

	public String getRnCd() {
		return rnCd;
	}

	public void setRnCd(String rnCd) {
		this.rnCd = rnCd;
	}

	public String getSigCd() {
		return sigCd;
	}

	public void setSigCd(String sigCd) {
		this.sigCd = sigCd;
	}

	public Integer getUndFloCo() {
		return undFloCo;
	}

	public void setUndFloCo(Integer undFloCo) {
		this.undFloCo = undFloCo;
	}

	
	
}
