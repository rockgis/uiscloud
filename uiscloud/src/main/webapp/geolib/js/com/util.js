...
function gfnDeactiveDoubleClick() {
...
파라메터 : _uploadButton 			(필수)업로드 버튼 id 
		   ...
		   _actionUrl				(옵션)기본은 '/titan/fileUpload'
리턴값 : json
***********************************************************************/
function gfnAjaxUpload(_uploadButton, _beforeSubmitCallback, _compleateCallback, _userId, _actionUrl) {
	...
	new AjaxUpload(_uploadButton[0], {
		action : _actionUrl? _actionUrl: '/titan/fileUpload',
		data : {
...
function gfnAjaxExcel2ArrayInArray(_uploadButton, _beforeSubmitCallback, _compleateCallback) {
	...
	return new AjaxUpload(_uploadButton[0], {
		action : '/titan/excel2ArrayInArray',
		autoSubmit: true,
		responseType : 'json',
		onComplete : function(file, response) { //파일 업로드 성공하였을 때 수행될 함수
			if(!_compleateCallback) {
...
function gfnGetDownloadUrl(name, userId, bThumbnail) {
	...
	var downloadUrl = "";
	downloadUrl += "/titan/fileDownload?requestedFile=";
	downloadUrl += name;
	...
	if(sidoListData) {
		sidoComboBox.combobox({
			...
			onSelect : function(record){
				if(sggComboBox != null) {
					setSggComboBox(record.ctprvnCd, sggComboBox, emdComboBox);
				}
...
function setSggComboBox(lglCd, sggComboBox, emdComboBox) {
    $.get(
    	"/gis/address/selectSggBySidoLglCd?ctprvnCd="+lglCd,
    	function(response) {
    		if(response && response.result) {
    			sggComboBox.combobox({
    				data: response.result,
    				...
    				onSelect : function(record){	
    					...
    					if(emdComboBox != null) 
						{
    						setEmdComboBox(record.sigCd, emdComboBox);
						}
    				}
    			});
    		}
...
function setEmdComboBox(lglCd, emdComboBox) {
    ...
    $.get(
    	"/gis/address/selectEmdlBySggLglCd?sigCd="+lglCd,
    	function(response) {
    		if(response && response.result) {
    			emdComboBox.combobox({
    				data: response.result,
 ...
 * @returns {String}
 */
function gfnFormatMgno(value, row) {
	return row.unqMgno + "/" + row.fctsNm;
}
 ...
 * @returns {Number}
 */
function gfnFormatRemandCoreCnt(value, row, index) {
	return row.coreCnt - row.useCoreCnt;
} 
 ...
 * @param index
 */
function gfnFormatUseCoreRatio(value, row, index) {
	var ratio = 0;
	if(row.coreCnt) {
		ratio = row.useCoreCnt / row.coreCnt * 100;
	}
	return Math.round(ratio) + "%";
}

function gfnFormatSysClf(value, row, index) {
	if(value == "SK") {
		return "SKT";
	}
	else if(value == "HT" || value == "TN") {
		return "SKB";
	}
	else if(value = "KE") {
		return "KEPCO";
...
function gfnFormatNumberOcppole(value, row, index) {
	if(row["isJp"] == "Y") {
		return gfnFormatNumber(value, row, index);
	}
	else {
		return "";
	}
 ...
 * @param style
 */
function gfnExtendSymbolizer(style, type) {
	if(!type) type = "default";
	return $.extend({}, OpenLayers.Feature.Vector.style[type], style);
}

/**
 * propertygrid 에서 입력받은 데이터를 전송할 데이터로 변환
 * @param data propertygrid 에 입력한 데이터
 * @returns {Object} 파라미터 객체
 */
function gfnGetParamsFromPropertyGridData(data) {
	var params = {};
	for(var i in data.rows) {
		params[data.rows[i].field] = data.rows[i].value;
	}
	return params;
}
 ...
 * 도형을 WKT 문자열로 변환
 * @param feature 도형정보
 * @returns {String} WKT 문자열
 */
function gfnGetWktFromFeature(feature) {
