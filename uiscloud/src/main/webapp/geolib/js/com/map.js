...
function gfnInitMap(div) {
    ...
    var view = new ol.View({
		...
		center : [14163425.793355, 4302305.8698004],
		zoom : 7,
		minZoom : 3,
		maxZoom : 17
	});
...
리턴값 :
***********************************************************************/
function gfnActiveControl(id, mapObj) {
	if(mapObj == null) {
		mapObj = map;
	}
...
/**********************************************************************
설명 : 지도 기능 전환
파라메터 : id - 활성할 인터랙션 아이디
			 mapObj - 지도 객체 (null 일 경우 메인 지도 객체 사용)
리턴값 :
***********************************************************************/
function gfnActiveInteraction(id, mapObj) {
	if(mapObj == null) {
		mapObj = map;
	}
...
/**********************************************************************
설명 : 지도 기능 전환
파라메터 : id - 비활성할 인터랙션 아이디
			 mapObj - 지도 객체 (null 일 경우 메인 지도 객체 사용)
리턴값 :
***********************************************************************/
function gfnActiveFalseInteraction(id, mapObj) {
	if(mapObj == null) {
		mapObj = map;
	}
...
/**********************************************************************
설명 : 배경 맵 레이어 추가 (vworld)
파라메터 : mapObj - 레이어를 등록할 Map 객체
리턴값 :
***********************************************************************/
function gfnAppendBaseLayer(mapObj) {
	gfnAppendVworldBaseLayer(mapObj);
	gfnAppendVworldSatelliteHybridLayer(mapObj);
	gfnAppendVworldSatelliteLayer(mapObj);
	
	gfnAppendDaumBaseLayer(mapObj);
	gfnAppendDaumSatelliteHybridLayer(mapObj);
	gfnAppendDaumSatelliteLayer(mapObj);
	
	gfnAppendNaverBaseLayer(mapObj);
	gfnAppendNaverSatelliteHybridLayer(mapObj);
	gfnAppendNaverSatelliteLayer(mapObj);
	
}

/**********************************************************************
설명 : Vworld Base 레이어 추가
파라메터 : mapObj - 레이어를 등록할 Map 객체
리턴값 :
***********************************************************************/
function gfnAppendVworldBaseLayer(mapObj) {
...
/**********************************************************************
설명 : Vworld 위성 + 시설물 레이어 추가
파라메터 : mapObj - 레이어를 등록할 Map 객체
리턴값 :
***********************************************************************/
function gfnAppendVworldSatelliteHybridLayer(mapObj) {
	var layer  = new ol.layer.Tile({
		source : new ol.source.VWorld({
			type : "hybrid"
		}),
...
리턴값 :
***********************************************************************/
function gfnAppendVworldSatelliteLayer(mapObj) {
	var layer  = new ol.layer.Tile({
		source : new ol.source.VWorld({ 
			type : "satellite"
		}),
...
리턴값 :
***********************************************************************/
function gfnAppendDaumBaseLayer(mapObj) {
	var layer  = new ol.layer.Tile({
...
/**********************************************************************
설명 : Daum 위성 + 시설물 레이어 추가
파라메터 : mapObj - 레이어를 등록할 Map 객체
리턴값 :
***********************************************************************/
function gfnAppendDaumSatelliteHybridLayer(mapObj) {
	var layer  = new ol.layer.Tile({
		source : new ol.source.Daum({
			type : "hybrid" 
		}),
...
/**********************************************************************
설명 : Naver Base 레이어 추가
파라메터 : mapObj - 레이어를 등록할 Map 객체
리턴값 :
***********************************************************************/
function gfnAppendNaverBaseLayer(mapObj) {
	var layer  = new ol.layer.Tile({
...
/**********************************************************************
설명 : Naver 위성 + 시설물 레이어 추가
파라메터 : mapObj - 레이어를 등록할 Map 객체
리턴값 :
***********************************************************************/
function gfnAppendNaverSatelliteHybridLayer(mapObj) {
	var layer  = new ol.layer.Tile({
		source : new ol.source.Naver({
			type : "hybrid"
		}),
...
리턴값 :
***********************************************************************/
function gfnAppendNaverSatelliteLayer(mapObj) {
	var layer  = new ol.layer.Tile({
		source : new ol.source.Naver({
			type : "satellite"
		}),
		name : "naver_satellite",
		visible : false
	});
	mapObj.addLayer(layer);
}
...
function gfnAppendWmsLayer(mapObj, name, layers) {
	...
	var layer = new ol.layer.Image({
		...
		source: new ol.source.ImageWMS({
	      ...
	      params: {
	          'LAYERS': layers,
	    	  //'LAYERS' : 'sktbigis:gnet_ring_map,sktbigis:gotc_ca,sktbigis:gotc_ca,sktbigis:gotc_ca,sktbigis:gotc_ca,sktbigis:gotc_ca,sktbigis:gotc_ca,sktbigis:gotc_tpo,sktbigis:gotc_tpo,sktbigis:gotc_tpo,sktbigis:gotc_tpo',
	    	  //'STYLES' : 'RM001,CA007,CA006,CA005,CA003,CA002,CA001,TPO004,TPO005,TPO002,TPO001',
	    	  //'LAYERS' : 'titan:GOTC_CA,titan:GOTC_TPO,titan:GOTC_JP,titan:GOTC_MH,titan:GOTC_CD,titan:GHFC_EP,titan:GOTC_SB,titan:GOTC_RF,titan:GOTC_SO,titan:GOTC_RT',
	          'SRS' : 'EPSG:900913',
	          'FORMAT' : 'image/png'
	      },
...
function gfnVectorLayer(mapObj){
...
function gfnMoveMaxExtent(mapObj) {
	...
	mapObj.getView().setCenter([14163425.793355, 4302305.8698004]);
	mapObj.getView().setZoom(7);
}
...
function gfnWFSGetFeature(featureType, filter, callback, options) {
	...
	var params = {
		...
		if(filter.fid){
			olCustom.service.WFS.getFeatureByObjectId(featureType, filter.fids, callback, params);
		}
		else{
			olCustom.service.WFS.getFeatureByDWithin(featureType, filter.distance, filter.value, callback, params);
		}
		
		//olCustom.service.WFS.getFeatureByComparison(featureType, "==", "gid", 9, callback, params);
}
...
/**********************************************************************
설명 : 속성 비교연산(Comparison)을 검색조건으로 검색
파라메터 : featureType - 검색할 도형타입 (테이블명)
			type 	- 검색할 연산식 타입
			 fields - 검색할 필드명 or 검색할 필드명의 목록
			 values - 검색할 값 or 검색할 필드값의 목록
...
function gfnGetFeatureById(featureType, values, bShow, bClean, bMove, callback, options) {
    ...
    gfnWFSGetFeature(featureType, filter, function(response) {
    	if(response && response.length > 0) {
    		if(bShow) {
    			gfnHighlightFeature(response, bClean, bMove, options);	
        	}	
    	}
    	else {
    		$.messager.alert("알림", "WFS GetFeature 실패");
...
//	}
	...
	if(bMove) {
		if(options && options.zoomLevel) {
			...
			var bounds = layer.getSource().getExtent();
...
function gfnDownloadKML(features) {
	...
	$("#frm_file_download").attr("action", "/titan/KMLFileDownload");
...
function gfnSaveMap() {
	...
	$("#frm_file_download").attr("action", "/titan/saveMapDownload");
	$("#frm_file_download").submit();*/
}
