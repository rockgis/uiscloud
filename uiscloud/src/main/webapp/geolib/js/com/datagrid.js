	...
	***********************************************************************/
	showResult : function(title, tableName, datas, newColumns, isPagination, onClickRow, onDblClickRow) {
		...
		if(newColumns) {
			...
			columns = that.createColumns(tableName, datas);
		}
...
//			}]
	...
	리턴값 :
	***********************************************************************/
	createColumns : function(tableName, datas) {
		var that = this;
		...
		if(tableName.toLowerCase() == "tl_spbd_buld_11000") {
			newColumns = [];
			var tmp = [];
			for(var i in datas.rows[0]){
				tmp.push({field:i,title:i, width:100, halign:"center", align:"right"});
			}
			newColumns.push(tmp);
		}
