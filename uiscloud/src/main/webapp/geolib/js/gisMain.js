...
$(document).ready(function() {	
	...
	drawTool.init(map);			// 그리기 도구 초기화 - /titan/js/drawTool/drawTool.js
	
	editTool.init(map);			// 편집 도구 초기화 - /titan/js/editTool/editTool.js
	...
	gfnVectorLayer(map);		// 벡터레이어 추가 /titan/js/com/map.js
	...
	gfnActiveInteraction(["defaultInteraction"]);
	
	$("#div_layout").layout("collapse", "south");
	
	// 테스트를 위한 초기위치 설정
	map.getView().setCenter([14143025.793355, 4512305.8698004]);
...
function gfnMenuEvent() {	
	...
	// 최단경로
	$("#a_menu_pgKRouting").click(function(){
		$("#div_view").accordion('select','최단경로 검색');
		// 라우팅 레이어 off
		if(!dynamicSld.showSld.is(":checked")){
			dynamicSld.showSld.attr("checked",true);
		}
	...
	// 다중경로
	$("#a_menu_pgMRouting").click(function(){
	...
	$("#a_menu_bbs").click(function(){
		...
		window.open("/titan/jpManage/draw2d", "jpManage").focus();
		return false;
	});	

	// 선번장
	$("#a_menu_jpManage").click(function(){
		window.open("/titan/jpManage", "jpManage").focus();
		return false;
	});
