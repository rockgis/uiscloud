...
function gfnInitMap(div) {
	...
	var map_config = "nextG.xml";
	var map_Hostconfig = "192.168.0.107:8880";

	var serviceUrl = "http://"+map_Hostconfig+"/geonuris/wms?GDX=nextG.xml"   // CACHE_KEY=sggiskey&FIXED=TRUE&GRAPHIC_BUFFER=0&ANTI=true&TEXT_ANTI=true&LABEL=HIDE_OVERLAP&REPEAT=null&DISPLAY_GAP=null";  //http://221.148.35.65:8880/geonuris/wms?GDX=raise_01.xml&FIXED=TRUE&REQUEST=GetCapabilities&SERVICE=WMS
	var serviceWfsUrl = "http://"+map_Hostconfig+"/geonuris/wfs?GDX=nextG.xml&";  //http://221.148.35.65:8880/geonuris/wms?GDX=raise_01.xml&FIXED=TRUE&REQUEST=GetCapabilities&SERVICE=WMS


	var projection = ol.proj.get('EPSG:5181');
	projection.setExtent([-219825.99, -535028.96, 819486.07, 777525.22]);

	var maxBounds = [-207603.04292279,-16362.359024006,677336.48694721,581569.75575303];//[-7373.148, -42048.4027, 631902.9514, 569360.017];

	var centerXY=[190504,252929];
	...
	var wmsLayer= new ol.layer.Image({
	    extent: maxBounds,
	    //maxResolution: 664.3690164189179,
...
function gfnActiveControl(controls, mapObj) {
	...
	for(var i in mapObj.controls) {
		if(mapObj.controls[i].type != OpenLayers.Control.TYPE_TOGGLE) {
...
function gfnMoveMaxExtent(mapObj) {
	...
	mapObj.setCenter(new OpenLayers.LonLat(266205, 308490), 1);
}
...
리턴값 : 
***********************************************************************/
function gfnWFSGetFeature(featureType, filter, callback) {
	
	$('#loadingBar').show();
	 // 토지 검색
    var protocol = new OpenLayers.Protocol.WFS.v1_1_0({
        url: "/bies/proxy?Service=WFS",
...
function gfnGetFeatureByDWithin(featureType, geometry, dist, bShow, bClean, bMove, callback) {
	var filter = new OpenLayers.Filter.Spatial({
...
function gfnDownloadKML(features) {
	...
	$("#frm_file_download").attr("action", "/bies/KMLFileDownload");
...
리턴값 : 버퍼된 공간영역 반환
***********************************************************************/
function gfnBufferGeometry(geometry, distance) {
...
function gfnSaveMap() {
	...
	$("#frm_file_download").attr("action", "/bies/saveMapDownload");
	$("#frm_file_download").submit();
}
