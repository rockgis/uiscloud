...
var bookmark = {
	...
	// 본부
	headOffice: $("#frmUploadBookmark .headOffice"),
	
	// 제목
	title: $("#frmUploadBookmark .title"),
	
	// 작성자명 
	creator: $("#frmUploadBookmark .creator"),
	
	searchButton: $("#frmUploadBookmark .search"),
	
	resetButton: $("#frmUploadBookmark .reset"),
	
	bookmarkExcelPath: $("#div_bookmark #bookmarkExcelPath"),
...
//		alert(message);		
		...
		var targetUrl = "/gis/bookmark/search";
...
//	    	    		$("#" + gridId).datagrid("getPanel").panel('refresh', '/gis/mapInfo/search?' + queryString);
	...
	compleateUploadExcel: function (file, response) {
		...
		default:
			...
			case 730:
				...
				$('#frm_bookmark_error_detail input[name="excelErrorMessage"]').val(response.excelErrorMessage);			
				$('#frm_bookmark_error_detail input[name="excel"]').val(excel);			
				$("#frm_bookmark_error_detail").submit();
				break;
			case 720:
	...
	onClickRow: function(rowIndex, rowData) {
		...
		$.ajax({
			type: "POST",
			url: "/gis/bookmark/selectBookmarkDetail",
			data: {
				  'headOffice': rowData.headOffice
				, 'creator': rowData.creator
				, 'title': rowData.title
			},
			dataType: "json",
			success : function(data) {
				var highers = data.highers;
				console.log(highers)
				bookmarkRoutingSearch.addHighers(highers);
				bookmarkRoutingSearch.addLowers(highers, data.lowers);
			}
		});
	},
	...
	onDblClickRow: function(rowIndex, rowData) {
		...
		$('#frm_bookmark input[name="headOffice"]').val(rowData.headOffice);
		$('#frm_bookmark input[name="creator"]').val(rowData.creator);
		$('#frm_bookmark input[name="title"]').val(rowData.title);
