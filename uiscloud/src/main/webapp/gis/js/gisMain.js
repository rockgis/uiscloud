...
$(document).ready(function() {	
	...
	gfnLoadingBar();			// 로딩바 - /gis/js/com/util.js
	gfnDeactiveRightClick();	// 마우스 우클릭 막기 - /gis/js/com/util.js
	
	gfnInitLayout();			// 공통화면 레이아웃
	gfnInitMainMap();			// 메인지도 초기화 - /gis/js/map/mainMap.js

	layerTree.init();			// 레이어 트리 - /gis/js/mapInfo/layerTree.js - 레이어 ON/OFF 이벤트 연결	
	
	mapInfo.init();				// 지도정보 - /gis/js/mapInfo/mapInfo.js
	
	dynamicSld.init();			// 지도정보-동적시설물결과보기 /gis/js/mapInfo/dynamicSld.js
	gfnSearchLayer(map);		// 검색결과 표시 레이어 /gis/js/com/map.js
	
	
	pgRoutingSearch.init();		// 단일경로 검색/기설루트 검색 - /gis/js/pgRouting/pgRoutingSearch.js

	//kRoutingSearch.init();		// 다중경로 검색/기설루트 검색 - /gis/js/kRouting/kRoutingSearch.js
	
	gfnMenuEvent();				// 상단 메뉴 이벤트 - /gis/js/gisMain.js
	//interactive.init();			// 인터렉티브UI 초기화
...
function gfnMenuEvent() {	
	...
	$("#a_menu_pgRouting").click(function(){
		...
		pgRoutingSearch.bbox = "";
		
		// clear bbox&Polygon
		pgRoutingSearch.clearBbox();
				
		// 라우팅 레이어 on
		pgRoutingSearch.turnLayersOn();

		if(dynamicSld.showSld.is(":checked")){
			dynamicSld.showSld.attr("checked",false);
		}
		return false;
	});	
	
	// 다중경로 검색 - 다중경로
	$("#a_menu_kRouting").click(function(){
		$("#div_view").accordion('select','다중경로 검색');
		kRoutingSearch.bbox = "";
		
		// clear bbox&Polygon
		kRoutingSearch.clearBbox();
		
		// 라우팅 레이어 on
		kRoutingSearch.turnLayersOn();
		if(dynamicSld.showSld.is(":checked")){
			dynamicSld.showSld.attr("checked",false);
		}
		return false;
	...
	// Interactive UI - D3
	$("#a_menu_interactiveUi").click(function(){
		interactive.popup = window.open("/gis/interactiveUi/show", "interactive", "top=50px, left=50px, width=1200px, height=700px, menubar=no, resizable=no, status=no, titlebar=no, toolbar=no, scrollbars=yes");
		interactive.popup.focus()
		return false;
	});	
	
	// Q&A
	$("#a_menu_bbs").click(function(){
		window.open("/gis/bbs/Bbs", "Q&A", "top=50px, left=50px, width=1200px, height=700px, menubar=no, resizable=no, status=no, titlebar=no, toolbar=no, scrollbars=yes").focus();
		return false;
	});	
	
	// 공지사항
	$("#a_menu_notice").click(function(){
		window.open("/gis/bbs/BbsList", "Q&A", "top=50px, left=50px, width=1200px, height=700px, menubar=no, resizable=no, status=no, titlebar=no, toolbar=no, scrollbars=yes").focus();
		return false;
	});	
	
	// 관리자
	$("#a_menu_admin").click(function(){
		window.open("/gis/memberList", "관리자", "top=50px, left=50px, width=820px, height=480px, menubar=no, resizable=no, status=no, titlebar=no, toolbar=no, scrollbars=yes").focus();
		return false;
	});
