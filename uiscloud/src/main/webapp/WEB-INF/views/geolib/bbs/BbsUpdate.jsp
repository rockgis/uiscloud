...
<script type="text/javascript" src="<c:url value='/titan/js/com/util.js'/>"></script>
<script type="text/javascript"	src="<c:url value='/titan/js/bbs/emailingCertService.js'/>"></script>
<script src="<c:url value='/titan/js/bbs/attachFile.js'/>"></script>
<script type="text/javaScript" defer="defer">
	...
	function fn_egov_selectList() {
		document.getElementById("updateForm").action = "<c:url value='/titan/bbs/Bbs'/>";
	...
	function fn_egov_save() {
		...
		frm.action = '<c:url value="/titan/bbs/updateBbs" />';
		frm.submit();
	}
	...
	function fn_egov_delete() {
		...
		frm.action = '<c:url value="/titan/bbs/deleteBbs" />';
		frm.submit();
	}
	...
	function fn_bbstail_save() {
		...
		frm.action = "<c:url value='/titan/bbsTail/addBbsTail'/>";
		frm.submit();
	}
	...
	function fn_bbstail_update(frmName) {
		...
		frm.action = "<c:url value='/titan/bbsTail/updateBbsTail'/>";
		frm.submit();
	}
	...
	function fn_bbstail_delete(frmName) {
		...
		frm.action = "<c:url value='/titan/bbsTail/deleteBbsTail'/>";
		frm.submit();
	}
	...
	function fn_egov_download(fileSeq) {
		...
		document.getElementById("fileForm").action = "<c:url value='/titan/bbs/downloadfile'/>";
	...
	function fn_egov_deleteFile(fileSeq) {
		...
		document.getElementById("fileForm").fileSeq.value = fileSeq;
		document.getElementById("fileForm").action = "<c:url value='/titan/bbs/deletefile'/>";
		document.getElementById("fileForm").submit();
	}
</script>
</head>
...
<div class="row" style="margin: 30px;">
	...
	<form:form commandName="bbsVO" name="updateForm" id="updateForm" class="form-horizontal"  enctype="multipart/form-data">
		...
		<div class="control-group">
			...
			<c:forEach var="file" items="${fileVO}" varStatus="attachLoop">		
				<div class="controls">				
            		...
            		<a href="#" onclick="javascript:fn_egov_deleteFile(<c:out value="${file.fileSeq}"/>)"><img alt="파일삭제" src="/titan/images/btn/close.png"></a>
		...
		<div class="row-fluid">
			<div class="span12 text-right">
				<a href="#" onclick="fn_egov_save();"><img src="/titan/images/bbs/btn_modify.png" alt="수정" /></a>
				<a href="#" onclick="fn_egov_delete();"><img src="/titan/images/bbs/btn_delete.png" alt="삭제" /></a>
				<a href="#" onclick="fn_egov_selectList();"><img src="/titan/images/bbs/btn_list.png" alt="목록" /></a>
			</div>
		</div>
	</form:form>
	<div class="row-fluid">
	...
	<c:forEach var="bbsTailVO" items="${bbsVO.bbsTailVOs}" varStatus="status">
		<form:form commandName="bbsTailVO" name="bbsTailEditForm${idx}"
			...
			<div class="row-fluid">
				<div class="span12 text-right">
					<a href="#" onclick="fn_bbstail_update('bbsTailEditForm${idx}');"><img src="/titan/images/bbs/btn_modify.png" alt="수정" /></a>
					<a href="#" onclick="fn_bbstail_delete('bbsTailEditForm${idx}');"><img src="/titan/images/bbs/btn_delete.png" alt="삭제" /></a>
				</div>
			</div>
		</form:form>
	...
	<form:form commandName="bbsTailVO" name="bbsTailDetailWriteForm"
		...
		<div class="row-fluid">
			<div class="span12 text-right">
				<a href="#" onclick="fn_bbstail_save();"><img src="/titan/images/bbs/btn_enter.png" alt="등록" /></a>
			</div>
		</div>
	</form:form>
</div>
